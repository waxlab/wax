...
--|
--| Different from the C `getopt()` or the GNU convention, `wax.getopt`
--| looks for a simpler way of handling command line arguments.
--|
--| **The result table**
--|
--| The return of parsing functions is a table. Consider the below
...
--| 2. Option can have a value or just "be present". When an options
--| doesn't have a value it is evaluated to an empty table.
--| 3. The same option can occur multiple times. If it is followed by
--| a value it will be stored under a table.
--| 4. Processing stops after the presence of double hyphen `--`
--| argument or after an argument that doesn't fill an option value.
