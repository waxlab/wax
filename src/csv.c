...
int
luaopen_wax_csv(lua_State *L) {
	l8n_newuserdata_mt(L, UD_CSV, ud_csv_mt);
	l8n_export(L, "wax_csv", module);
	return 1;
}
...
wax_csv_lists(lua_State *L) {
	struct ud_csv *u = luaL_checkudata(L, 1, UD_CSV);
	l8n_assert(L, aux_reset(u), strerror(errno));
	aux_nextchar(u);
	lua_pushvalue(L,1);
...
wax_csv_records(lua_State *L) {
	struct ud_csv *u = luaL_checkudata(L, 1, UD_CSV);
	l8n_assert(L, aux_reset(u),        strerror(errno));
	l8n_assert(L, aux_allockeys(u,2), strerror(errno));
	aux_nextchar(u);
	...
	if (lua_gettop(L) < 2) { /* first row field as result key */
		...
		do {
			...
			c8l_arrpush(u->val,'\0');
			field = c8l_arrnew(*field, c8l_arrlen(u->val));
			strcpy(field, u->val);
			l8n_assert(L, c8l_arrpush(u->keys, field), strerror(errno));
		} while(noeor);
		...
		for (k=1, l=l8n_rawlen(L,-2); k <= l; k++) {
			lua_rawgeti(L,-1,k);
			l8n_assert(L, c8l_arrpush(u->keys, (char *)luaL_checkstring(L,-1)), strerror(errno));
			lua_pop(L,1);
		}
		lua_pop(L,1);
...
aux_reset(struct ud_csv *u) {
	...
	if (u->keys != NULL) c8l_arrfree(u->keys);
...
aux_allockeys(struct ud_csv *u, size_t len) {
	...
	if (u->keys != NULL) {
		aux_resetkeys(u);
		return c8l_arrcapsz(u->keys, len);
	}

	u->keys = c8l_arrnew(u->keys, len);
